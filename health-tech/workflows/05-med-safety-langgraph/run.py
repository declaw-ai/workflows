...
safety advisory JSON. The agent orchestrates a tool chain against real
public APIs:

  1. fetch_patient_meds           (local FHIR mock)
  2. drug_interactions_from_fda   → openFDA label (real HTTP)
  3. openfda_adverse_events       → openFDA event counts (real HTTP)
  4. synthesize_advisory          → gpt-4.1 (real LLM)
...
from shared.external_apis import (  # noqa: E402
    drug_interactions_from_fda, openfda_adverse_events,
)
