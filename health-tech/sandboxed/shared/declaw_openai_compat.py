...
Safe:
    * Only flips the default when Accept-Encoding is unset; explicit values
      set by callers are preserved.
    * Leaves streaming calls (SSE) unchanged -- they're already uncompressed.
    * Also patches AsyncClient so async callers get the same behaviour.

Streaming is not affected because SSE responses are already uncompressed
in practice; the proxy handles those via chunk-boundary buffering.
"""
...
def _inject_default(headers_arg):
    """Return a Headers object with Accept-Encoding=identity unless the
...
# Optional introspection for tests
def is_patched() -> bool:
