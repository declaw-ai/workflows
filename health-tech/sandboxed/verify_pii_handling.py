...
What we expect to see (current declaw build, observed 2026-04):
                          rehydrate=False         rehydrate=True
    httpbin destination   tokens                  originals
    OpenAI  destination   tokens                  tokens   <-- not rehydrated

Conclusion: outbound redaction is reliable on both destinations.
Inbound rehydration works for plain HTTP/JSON destinations but does NOT
fire on OpenAI's chunked/compressed response stream in this build.
Production code that wants the agent to see original PHI back from an
OpenAI call should either (a) use rehydrate-friendly destinations,
(b) wait for declaw to add OpenAI streaming-aware rehydration, or
(c) re-attach identifiers in the orchestrator from the chart it already
holds.
"""
