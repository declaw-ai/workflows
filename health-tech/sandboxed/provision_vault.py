"""Provision Declaw credential-vault secrets for the health-tech demos (optional).

By default the sandboxed workflows forward the host's OPENAI_API_KEY into the
microVM as an env var. The stronger posture is the credential vault: the real
key lives server-side in OpenBao and is injected by the egress proxy on the
...
This script provisions the OpenAI key into the vault using the built-in provider
preset — the preset supplies the domain scope and header injection rule (OpenAI:
Authorization: Bearer). It then prints the env export that switches the
workflows onto the vault path (`declaw_helpers.llm_vault_refs()` reads it):
...
Every sandboxed health-tech workflow calls OpenAI gpt-4.1 in-VM, so OpenAI is
the only LLM secret worth brokering here (no Anthropic on the sandboxed path).
"""
...
SECRETS = {
    "OPENAI_API_KEY": ("healthtech-openai", "openai"),
}
...
def provision() -> None:
    ...
    if exports:
        ...
        print("\nNo host keys found to provision (set OPENAI_API_KEY first).")
...
def delete() -> None:
    ...
    try:
        ...
        vault.close()
    print("\nUnset the ref to return to env forwarding:")
