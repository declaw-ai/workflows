...
| **`AuditConfig`** with structured event log | Per-action audit record (operator + agent + data + timestamp) — the kind of access/audit logging that HHS OCR's proposed 2025 HIPAA Security Rule update (NPRM; not yet final) moves toward. Events are recorded server-side and surfaced via the declaw dashboard / control-plane API. |
...
- Because the content source is actively adversarial-adjacent, this policy turns the injection cascade on at **`log_only`** — Tier-1 ML classifier (`injection_mode="data-egress-sensitive"`) + the Tier-2 LLM judge. Detections are audited without blocking legit traffic.
