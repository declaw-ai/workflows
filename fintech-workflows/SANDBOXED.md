...
- **04 chargeback baseline** places card PAN `4111 1111 1111 1111` + CVV `123` in the LLM prompt — a direct PCI-DSS v4.0 req 3.3.1 violation.
...
the **card CVV**, which is redacted but never rehydrated (PCI-DSS v4.0 req 3.3.1).
...
- **Regulatory evidence comes for free.** DPDP / RBI digital-lending / SEBI IA /
  FinCEN SAR / PCI-DSS v4.0 req 3.3.1 all want per-action logs of
...
| **CVV never rehydrated** (pci_payments_policy) + **PII action=`block`** (opt-in on kyc_document_policy / pci_payments_policy / tax_filing_policy) | The **card CVV** is redacted and never rehydrated — PCI-DSS v4.0 req 3.3.1 prohibits retaining it post-auth, even as a token round-trip. For identifier classes a deployment wants to *never* let leave the sandbox at all — **Aadhaar** (DPDP 'sensitive personal data'), **raw ledger lines** (proprietary IP) — flip the policy's PII `action` to `block` so the proxy drops the request instead of redacting-then-forwarding (the default posture in these demos is redact + rehydrate). |
...
- `draft_packet` runs under `pci_payments_policy`: card PAN redacted + rehydrated, but the **CVV is redacted and never rehydrated** (PCI-DSS v4.0 req 3.3.1). Allowlist = `api.stripe.com + api.openai.com` only. The `owasp-agentic@v1` governance pack guards the Stripe dispute tool.
