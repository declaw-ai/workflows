...
| 05 | `05_audit_log_api_renamed.py` | BY DESIGN — `AuditConfig(enabled=True)` records events server-side; no method on `Sandbox` exposes them. Retrieve via the Declaw dashboard / a separate control-plane API. File logged as a reminder + doc-request to add a client-side export helper. | Low (documentation + DX) |
...
4. **#05 audit export** — low-priority DX item: confirmed by design that no Sandbox-level retrieval method exists. Fix shape is a `declaw.audit.export(sandbox_id=..., since=..., until=...)` client helper, not a method on the Sandbox.
