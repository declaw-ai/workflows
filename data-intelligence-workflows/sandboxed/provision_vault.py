"""Provision Declaw credential-vault secrets for the data-intelligence demos (optional).
...
This vertical is analytics/BI over warehouse + SaaS data — the only high-value
secret is the OpenAI key, so that is all this script provisions. It uses the
built-in `openai` provider preset (domain scope + Authorization: Bearer
injection) and then prints the env export that switches the workflows onto the
vault path (`declaw_helpers.llm_vault_refs()` reads it):
...
SECRETS = {
    "OPENAI_API_KEY": ("data-intel-openai", "openai"),
}
