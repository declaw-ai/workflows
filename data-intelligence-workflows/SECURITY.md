...
- (optional) `InjectionDefenseConfig(enabled=True, action="log_only",
  injection_mode="data-egress-sensitive")` wrapping a Tier-2 LLM judge
...
| 02 Telemetry Fusion | 1 microVM (full FunctionAgent) | **`enable_injection_scan=True`** — `data-egress-sensitive` cascade + Tier-2 LLM judge; manual text is untrusted |
